 ...
 */
export function createBigQueryRouter(config: BigQueryRouterConfig): Router {
  const {
    client,
    authorizeQuery = defaultAuthorizeQuery,
  ...
  router.post('/query', async (req, res, next) => {
    try {
      const { sql, params, maxResults, timeoutMs } = QueryRequestSchema.parse(
        req.body
      );
